...
def get_result (train, test, k):
    ...
    validation_error,trainning_error,generalization_error = knn_main.cross_validation (J, train, knn_main.knn_try, seed, k, istraining)
    #print(trainning_error,generalization_error)
    result = knn_main.mean_performance(validation_error,trainning_error,generalization_error)
...
def other():
    ...
    writer = pd.ExcelWriter('Q2mean_error.xlsx', engine='xlsxwriter')
    ...
    print("Performance of each fold (average validation error): ",result[0])
    print("validation error 25 and 75 percentiles: ",result[1],result[2])
    if (knn_main.istraining):
        print("Average of Trainning error: ",result[3])
        print("Average of generalization error: ",result[4])
    y = knn_main.helper_find_y(knn_main.test)
    y_train = knn_main.helper_find_y(knn_main.train)
    validation_error = knn_main.evaluate_misclassify (knn_main.knn,test_data, y, knn_main.misclassify_rate, k, train_data)
    trainning_error = knn_main.evaluate_misclassify (knn_main.knn,train_data, y_train, knn_main.misclassify_rate, k, train_data)
    generalization_error = validation_error - trainning_error
    #result = mean_performance(validation_error,trainning_error,generalization_error)
    print("Performance of each fold (average validation error): ",validation_error)
    if (istraining):
